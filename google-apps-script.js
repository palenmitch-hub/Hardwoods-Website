...
function buildInventoryAdminPage() {
  ...
  return '<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Inventory Manager</title>' +
    ...
    'var KEY=' + key + ',products=[],$=function(id){return document.getElementById(id)},status=function(v){$("status").textContent=v},esc=function(v){return String(v||"").replace(/[&<>]/g,function(c){return{"&":"&amp;","<":"&lt;",">":"&gt;"}[c]})},call=function(method,args,done){var r=google.script.run.withSuccessHandler(done).withFailureHandler(function(e){status(e.message||String(e))});r[method].apply(r,args)},load=function(){call("getInventoryAdminData",[KEY],function(v){products=v||[];render()})},render=function(){$("list").innerHTML=products.map(function(p){var pics=(p.images||[]).map(function(n,i){return "<label class=photo><input type=checkbox value=\""+esc(n)+"\"><img src=\""+(p.imageUrls[i]||"")+"\" alt=\"Photo "+(i+1)+"\"></label>"}).join("");return "<article class=product data-number=\""+esc(p.productNum)+"\"><h3>"+esc(p.name)+"</h3><small>#"+esc(p.productNum)+" - $"+Number(p.price).toFixed(2)+" - "+p.qty+" available</small><p>"+esc(p.description||"No description")+"</p><div class=photos>"+pics+"</div><div class=actions><input id=\"add-"+esc(p.productNum)+"\" type=file accept=\"image/jpeg,image/png,image/webp\" multiple><button class=secondary onclick=addPhotos(\""+esc(p.productNum)+"\")>Add photos</button><button class=secondary onclick=removePhotos(\""+esc(p.productNum)+"\")>Remove selected</button><button class=danger onclick=sold(\""+esc(p.productNum)+"\")>Mark sold</button><button onclick=edit(\""+esc(p.productNum)+"\")>Edit</button></div></article>"}).join("")||"<p>No products found.</p>"},edit=function(n){var p=products.filter(function(x){return x.productNum===n})[0];$("title").textContent="Edit product #"+n;$("number").value=n;$("name").value=p.name;$("description").value=p.description||"";$("price").value=p.price;$("qty").value=p.qty;$("type").value=p.type||"cutting-board";window.scrollTo(0,0)},read=function(fs,done){var a=[],left=fs.length;if(!left)return done(a);Array.prototype.forEach.call(fs,function(f){var r=new FileReader();r.onload=function(){a.push({name:f.name,mimeType:f.type,base64:String(r.result).split(",")[1]});if(!--left)done(a)};r.readAsDataURL(f)})},addPhotos=function(n){read($("add-"+n).files,function(a){status("Adding photos...");call("addInventoryAdminImages",[n,a,KEY],function(){status("Photos added");load()})})},removePhotos=function(n){var c=document.querySelector("[data-number=\""+n+"\"]"),a=Array.prototype.map.call(c.querySelectorAll("input:checked"),function(x){return x.value});if(!a.length||!confirm("Remove selected photos?"))return;call("removeInventoryAdminImages",[n,a,KEY],function(){status("Photos removed");load()})},sold=function(n){if(!confirm("Mark product #"+n+" as sold?"))return;call("markInventoryAdminProductSold",[n,KEY],function(){status("Product marked sold");load()})};$("form").onsubmit=function(e){e.preventDefault();var d={productNum:$("number").value.trim(),name:$("name").value.trim(),description:$("description").value.trim(),price:$("price").value,qty:$("qty").value,type:$("type").value};read($("files").files,function(a){d.images=a;status("Saving...");call("saveInventoryAdminProduct",[d,KEY],function(){status("Product saved");load()})})};$("clear").onclick=function(){$("form").reset();$("title").textContent="Add product"};load();</script></body></html>';
}
