...
function getInventoryAdminData(key) {
  ...
  if (!manifest.length) {
    manifest = entries.filter(function (entry) {
      return entry && entry.type === 'file' && /\.(jpe?g|png|webp)$/i.test(entry.name || '');
  ...
  var products = {};
  var multiImagePattern = /^(.+)-(\d+(?:\.\d{1,2})?)-(\d+)-([A-Za-z0-9]+)-(\d+)\.(jpe?g|png|webp)$/i;
  var singleImagePattern = /^(.+)-(\d+(?:\.\d{1,2})?)-(\d+)-([A-Za-z0-9]+)\.(jpe?g|png|webp)$/i;

  for (var i = 0; i < manifest.length; i++) {
    var fileName = String(manifest[i] || '');
    var entry = entryByName[fileName];
    var match = fileName.match(multiImagePattern) || fileName.match(singleImagePattern);
    if (!match) continue;
    var number = match[4];
    if (!products[number]) {
...
function markInventoryAdminProductSold(productNum, key) {
  var cfg = requireInventoryAdmin_(key);
  ...
  recordSoldInventory_(productNum, result.removedFiles, 'Inventory Manager');
  logInventoryHistory_({ action: 'sold', productNumber: productNum, removedCount: result.removedCount, removedFromManifest: result.removedFromManifest, removedFiles: result.removedFiles, status: 'success' });
  return { success: true };
}

function removeInventoryAdminImages(productNum, fileNames, key) {
  var cfg = requireInventoryAdmin_(key);
  var wanted = Array.isArray(fileNames) ? fileNames : [];
  if (!wanted.length) throw new Error('Select at least one photo to remove.');
  var entries = listGitHubDirectory_(cfg, 'images/products/available');
  var removed = [];
  for (var i = 0; i < entries.length; i++) {
    var entry = entries[i];
    if (!entry || wanted.indexOf(entry.name) === -1) continue;
    deleteGitHubFile_(cfg, entry.path, entry.sha, 'Remove inventory image: ' + entry.name);
    removed.push(entry.name);
  }
  if (!removed.length) throw new Error('No selected photos were found.');
  var manifest = getInventoryManifestFromGitHub_(cfg).filter(function (name) { return removed.indexOf(name) === -1; });
  removeInventoryFromManifestOnGitHub_(cfg, String(productNum || '').trim().toLowerCase());
  updateInventoryManifestOnGitHub_(cfg, manifest);
  return { success: true, removed: removed };
}

function addInventoryAdminImages(productNum, images, key) {
  var cfg = requireInventoryAdmin_(key);
  var product = getInventoryAdminData(key).filter(function (item) { return String(item.productNum) === String(productNum); })[0];
  if (!product) throw new Error('Product #' + productNum + ' was not found.');
  if (!Array.isArray(images) || !images.length) throw new Error('Select at least one photo to add.');
  var parsed = { name: product.name, price: product.price, qty: product.qty, productNum: product.productNum };
  var added = [];
  for (var i = 0; i < images.length; i++) {
    var image = images[i] || {};
    if (!image.base64) continue;
    var ext = String(image.mimeType || '').split('/')[1] || 'jpeg';
    if (ext === 'jpeg') ext = 'jpg';
    var fileName = buildInventoryFileName(parsed, ext, product.images.length + added.length + 1, product.images.length + images.length);
    pushInventoryImageToGitHub_(cfg, fileName, Utilities.newBlob(Utilities.base64Decode(image.base64), image.mimeType || 'image/jpeg', image.name || fileName));
    added.push(fileName);
  }
  if (!added.length) throw new Error('No valid photos were selected.');
  updateInventoryManifestOnGitHub_(cfg, added);
...
function updateInventoryMetadataOnGitHub_(cfg, productNumber, metadata) {
  ...
  records[key] = {
    ...
    description: incoming.descriptionSpecified ? incoming.description : (previous.description || ''),
...
/* Legacy manager implementation retained below for reference only.
function buildInventoryAdminPageLegacy_() {
  ...
  return '<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">' +
    ...
    'var KEY=' + key + ',products=[];var $=function(id){return document.getElementById(id)};function status(text){$("status").textContent=text}function call(method,args,done){var runner=google.script.run.withSuccessHandler(done).withFailureHandler(function(e){$("products").innerHTML="<p>Could not load inventory: "+esc(e.message||String(e))+"</p>";status(e.message||String(e))});if(method==="getInventoryAdminData")runner.getInventoryAdminData(args[0]);else if(method==="removeInventoryAdminProduct")runner.removeInventoryAdminProduct(args[0],args[1]);else if(method==="removeInventoryAdminImages")runner.removeInventoryAdminImages(args[0],args[1],args[2]);else if(method==="markInventoryAdminProductSold")runner.markInventoryAdminProductSold(args[0],args[1]);else if(method==="saveInventoryAdminProduct")runner.saveInventoryAdminProduct(args[0],args[1]);}function load(){call("getInventoryAdminData",[KEY],function(data){products=data||[];render()})}function render(){var out="";products.forEach(function(p){var thumb=p.imageUrls&&p.imageUrls[0]?"<img src=\\\""+p.imageUrls[0]+"\\\" alt=\\\"\\\">":"<div></div>";var pics=(p.images||[]).map(function(image,i){return '<label class=photo><input type=checkbox value="'+esc(image)+'"><img src="'+(p.imageUrls[i]||'')+'" alt="Photo '+(i+1)+'"><span>'+esc(image)+'</span></label>'}).join('');out+="<article class=product>"+thumb+"<div><h3>"+esc(p.name)+"</h3><small>#"+esc(p.productNum)+" - $"+Number(p.price).toFixed(2)+" - "+p.qty+" available - "+esc(p.type)+"</small><p class=description>"+esc(p.description||"")+"</p><div class=photos>"+pics+"</div></div><div class=actions><button onclick=edit("+JSON.stringify(p.productNum)+")>Edit</button><button class=secondary onclick=removeImages("+JSON.stringify(p.productNum)+")>Remove photos</button><button class=danger onclick=sold("+JSON.stringify(p.productNum)+")>Mark sold</button><button class=danger onclick=removeProduct("+JSON.stringify(p.productNum)+")>Remove</button></div></article>"});$("products").innerHTML=out||"<p>No products found.</p>"}function esc(v){return String(v||"").replace(/[&<>]/g,function(c){return{"&":"&amp;","<":"&lt;",">":"&gt;"}[c]})}function edit(number){var p=products.filter(function(x){return x.productNum===number})[0];if(!p)return;$("form-title").textContent="Edit product #"+number;$("original-number").value=number;$("product-number").value=number;$("product-name").value=p.name;$("product-description").value=p.description||"";$("product-price").value=p.price;$("product-qty").value=p.qty;$("product-type").value=p.type||"cutting-board";$("opt-groove").checked=!!p.preAdded.juiceGroove;$("opt-handles").checked=!!p.preAdded.handles;$("opt-feet").checked=!!p.preAdded.feet;window.scrollTo(0,0)}function removeProduct(number){if(!confirm("Remove product #"+number+" from Available Now?"))return;status("Removing...");call("removeInventoryAdminProduct",[number,KEY],function(){status("Removed product #"+number);load()})}function removeImages(number){var p=products.filter(function(x){return x.productNum===number})[0];var selected=Array.prototype.filter.call(document.querySelectorAll('.product input:checked'),function(input){return input.closest('.product').querySelector('h3').textContent===p.name}).map(function(input){return input.value});if(!selected.length||!confirm("Remove selected photos from product #"+number+"?"))return;status("Removing photos...");call("removeInventoryAdminImages",[number,selected,KEY],function(){status("Removed selected photos");load()})}function sold(number){if(!confirm("Mark product #"+number+" as sold and remove it from Available Now?"))return;status("Marking sold...");call("markInventoryAdminProductSold",[number,KEY],function(){status("Marked product #"+number+" as sold");load()})}function clearForm(){$("product-form").reset();$("original-number").value="";$("form-title").textContent="Add product";$("product-images").value=""}function readImages(files,done){var result=[],left=files.length;if(!left)return done(result);Array.prototype.forEach.call(files,function(file){var reader=new FileReader();reader.onload=function(){result.push({name:file.name,mimeType:file.type,base64:String(reader.result).split(",")[1]});if(!--left)done(result)};reader.readAsDataURL(file)})}$("clear").onclick=clearForm;$("product-form").onsubmit=function(e){e.preventDefault();var data={productNum:$("product-number").value.trim(),name:$("product-name").value.trim(),description:$("product-description").value.trim(),price:$("product-price").value,qty:$("product-qty").value,type:$("product-type").value,juiceGroove:$("opt-groove").checked,handles:$("opt-handles").checked,feet:$("opt-feet").checked};status("Saving...");readImages($("product-images").files,function(images){data.images=images;call("saveInventoryAdminProduct",[data,KEY],function(result){status("Saved product #"+result.productNum);clearForm();load()})})};load();</script></body></html>';
}

// The manager page exposes descriptions, current photos, selective photo removal, and sold status.
function buildInventoryAdminPageLegacy2_() {
  var key = JSON.stringify(getInventoryAdminKey_());
  return '<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Inventory Manager</title>' +
    '<style>body{font:15px Arial,sans-serif;margin:0;background:#f4f1eb;color:#29251f}main{max-width:1100px;margin:auto;padding:28px 18px}.layout{display:grid;grid-template-columns:1fr 1fr;gap:22px}.panel{background:#fff;border:1px solid #d9d0c4;border-radius:8px;padding:20px}label{display:block;font-weight:700;margin:12px 0 5px}input,select,textarea{width:100%;box-sizing:border-box;padding:10px;border:1px solid #cfc4b6;border-radius:5px;font:inherit}textarea{min-height:90px;resize:vertical}input[type=checkbox]{width:auto}.checks,.photos,.actions{display:flex;gap:10px;flex-wrap:wrap}.checks label,.photo{font-weight:400;margin:0}.photo{display:flex;align-items:center;font-size:11px}.photo img{width:64px;height:48px;object-fit:cover;margin-right:4px}.product{border-top:1px solid #e4ddd4;padding:14px 0}.product:first-child{border-top:0}.product h3{margin:0 0 5px}.product small{color:#776e64}.product .actions{margin-top:10px}.product button{padding:8px 10px}button{border:0;border-radius:5px;padding:10px 15px;font-weight:700;cursor:pointer;background:#5b3d25;color:#fff}.secondary{background:#e8e0d6;color:#382b20}.danger{background:#a33b2e}.status{min-height:22px;margin-top:12px;color:#5b3d25}@media(max-width:720px){.layout{grid-template-columns:1fr}}</style></head><body><main><h1>Inventory Manager</h1><p>Edit product details, manage current photos, or mark a product sold.</p><div class="layout"><section class="panel"><h2 id="form-title">Add product</h2><form id="product-form"><input type="hidden" id="original-number"><label>Product number</label><input id="product-number" pattern="[A-Za-z0-9]+"><label>Name</label><input id="product-name" required><label>Description</label><textarea id="product-description"></textarea><label>Price</label><input id="product-price" type="number" min="0.01" step="0.01" required><label>Quantity</label><input id="product-qty" type="number" min="1" step="1" value="1" required><label>Type</label><select id="product-type"><option value="cutting-board">Cutting board</option><option value="charcuterie">Charcuterie</option><option value="other">Other</option></select><label>Already included</label><div class="checks"><label><input type="checkbox" id="opt-groove"> Juice Groove</label><label><input type="checkbox" id="opt-handles"> Handles</label><label><input type="checkbox" id="opt-feet"> Feet</label></div><label>Photos <small>(new photos replace current photos)</small></label><input id="product-images" type="file" accept="image/jpeg,image/png,image/webp" multiple><div class="actions"><button type="submit">Save product</button><button type="button" class="secondary" id="clear">Clear</button></div><div class="status" id="status" aria-live="polite"></div></form></section><section class="panel"><h2>Available products</h2><div id="products">Loading...</div></section></div></main><script>' +
    'var KEY=' + key + ',products=[],$=function(id){return document.getElementById(id)};function esc(v){return String(v||"").replace(/[&<>]/g,function(c){return{"&":"&amp;","<":"&lt;",">":"&gt;"}[c]})}function status(v){$("status").textContent=v}function call(method,args,done){var r=google.script.run.withSuccessHandler(done).withFailureHandler(function(e){status(e.message||String(e))});if(method==="getInventoryAdminData")r.getInventoryAdminData(args[0]);if(method==="saveInventoryAdminProduct")r.saveInventoryAdminProduct(args[0],args[1]);if(method==="removeInventoryAdminImages")r.removeInventoryAdminImages(args[0],args[1],args[2]);if(method==="markInventoryAdminProductSold")r.markInventoryAdminProductSold(args[0],args[1])}function load(){call("getInventoryAdminData",[KEY],function(v){products=v||[];render()})}function render(){var out="";products.forEach(function(p){var pics=(p.images||[]).map(function(n,i){return "<label class=photo><input type=checkbox value=\""+esc(n)+"\"><img src=\""+(p.imageUrls[i]||"")+"\" alt=\"Photo "+(i+1)+"\"><span>"+esc(n)+"</span></label>"}).join("");out+="<article class=product data-number=\""+esc(p.productNum)+"\"><h3>"+esc(p.name)+"</h3><small>#"+esc(p.productNum)+" - $"+Number(p.price).toFixed(2)+" - "+p.qty+" available - "+esc(p.type)+"</small><p>"+esc(p.description||"No description")+"</p><div class=photos>"+pics+"</div><div class=actions><button onclick=edit(\""+esc(p.productNum)+"\")>Edit</button><button class=secondary onclick=removePhotos(\""+esc(p.productNum)+"\")>Remove selected photos</button><button class=danger onclick=sold(\""+esc(p.productNum)+"\")>Mark sold</button></div></article>"});$("products").innerHTML=out||"<p>No products found.</p>"}function edit(n){var p=products.filter(function(x){return x.productNum===n})[0];if(!p)return;$("form-title").textContent="Edit product #"+n;$("original-number").value=n;$("product-number").value=n;$("product-name").value=p.name;$("product-description").value=p.description||"";$("product-price").value=p.price;$("product-qty").value=p.qty;$("product-type").value=p.type||"cutting-board";$("opt-groove").checked=!!p.preAdded.juiceGroove;$("opt-handles").checked=!!p.preAdded.handles;$("opt-feet").checked=!!p.preAdded.feet;window.scrollTo(0,0)}function removePhotos(n){var card=document.querySelector("[data-number=\""+n+"\"]"),selected=Array.prototype.map.call(card.querySelectorAll("input:checked"),function(x){return x.value});if(!selected.length||!confirm("Remove selected photos from product #"+n+"?"))return;status("Removing photos...");call("removeInventoryAdminImages",[n,selected,KEY],function(){status("Selected photos removed");load()})}function sold(n){if(!confirm("Mark product #"+n+" as sold and remove it from Available Now?"))return;status("Marking sold...");call("markInventoryAdminProductSold",[n,KEY],function(){status("Product marked sold");load()})}function clearForm(){$("product-form").reset();$("original-number").value="";$("form-title").textContent="Add product"}function readImages(files,done){var result=[],left=files.length;if(!left)return done(result);Array.prototype.forEach.call(files,function(f){var r=new FileReader();r.onload=function(){result.push({name:f.name,mimeType:f.type,base64:String(r.result).split(",")[1]});if(!--left)done(result)};r.readAsDataURL(f)})}$("clear").onclick=clearForm;$("product-form").onsubmit=function(e){e.preventDefault();var d={productNum:$("product-number").value.trim(),name:$("product-name").value.trim(),description:$("product-description").value.trim(),price:$("product-price").value,qty:$("product-qty").value,type:$("product-type").value,juiceGroove:$("opt-groove").checked,handles:$("opt-handles").checked,feet:$("opt-feet").checked};status("Saving...");readImages($("product-images").files,function(images){d.images=images;call("saveInventoryAdminProduct",[d,KEY],function(){status("Product saved");clearForm();load()})})};load();</script></body></html>';
}
...
function buildInventoryAdminPage() {
  var key = JSON.stringify(getInventoryAdminKey_());
  return '<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Inventory Manager</title>' +
    '<style>body{font:15px Arial;margin:0;background:#f4f1eb;color:#29251f}main{max-width:1100px;margin:auto;padding:28px 18px}.layout{display:grid;grid-template-columns:1fr 1fr;gap:22px}.panel,.product{background:#fff;border:1px solid #d9d0c4;border-radius:8px;padding:18px}.product{margin:12px 0}label{display:block;font-weight:700;margin:10px 0 4px}input,select,textarea{box-sizing:border-box;width:100%;padding:9px;font:inherit;border:1px solid #cfc4b6;border-radius:5px}textarea{min-height:80px}.checks,.photos,.actions{display:flex;gap:8px;flex-wrap:wrap}.checks label,.photo{font-weight:400;margin:0}.photo{font-size:11px;align-items:center}.photo img{width:64px;height:48px;object-fit:cover}.actions{margin-top:12px}button{padding:9px 12px;border:0;border-radius:5px;background:#5b3d25;color:#fff;font-weight:700;cursor:pointer}.secondary{background:#e8e0d6;color:#382b20}.danger{background:#a33b2e}.status{min-height:22px;margin-top:10px}@media(max-width:720px){.layout{grid-template-columns:1fr}}</style></head><body><main><h1>Inventory Manager</h1><p>Edit descriptions, add or remove photos, and mark products sold.</p><div class="layout"><section class="panel"><h2 id="title">Add product</h2><form id="form"><input id="number" placeholder="Product number"><label>Name</label><input id="name" required><label>Description</label><textarea id="description"></textarea><label>Price</label><input id="price" type="number" min=".01" step=".01" required><label>Quantity</label><input id="qty" type="number" min="1" required><label>Type</label><select id="type"><option value="cutting-board">Cutting board</option><option value="charcuterie">Charcuterie</option><option value="other">Other</option></select><label>Replace all photos on save</label><input id="files" type="file" accept="image/jpeg,image/png,image/webp" multiple><div class="actions"><button>Save product</button><button type="button" class="secondary" id="clear">Clear</button></div><div class="status" id="status"></div></form></section><section class="panel"><h2>Available products</h2><div id="list">Loading...</div></section></div></main><script>' +
    'var KEY=' + key + ';var products=[];var $=function(id){return document.getElementById(id)};' +
    'function status(v){$("status").textContent=v}' +
    'function esc(v){return String(v||"").replace(/[&<>]/g,function(c){return{"&":"&amp;","<":"&lt;",">":"&gt;"}[c]})}' +
    'function call(method,args,done){' +
      'var r=google.script.run.withSuccessHandler(done).withFailureHandler(function(e){status(e.message||String(e))});' +
      'if(method==="getInventoryAdminData")r.getInventoryAdminData(args[0]);' +
      'else if(method==="saveInventoryAdminProduct")r.saveInventoryAdminProduct(args[0],args[1]);' +
      'else if(method==="removeInventoryAdminImages")r.removeInventoryAdminImages(args[0],args[1],args[2]);' +
      'else if(method==="addInventoryAdminImages")r.addInventoryAdminImages(args[0],args[1],args[2]);' +
      'else if(method==="markInventoryAdminProductSold")r.markInventoryAdminProductSold(args[0],args[1]);' +
    '}' +
    'function load(){call("getInventoryAdminData",[KEY],function(v){products=v||[];render()})}' +
    'function render(){$("list").innerHTML=products.map(function(p){' +
      'var pics=(p.images||[]).map(function(n){return "<label class=photo><input type=checkbox value=\\""+esc(n)+"\\"> "+esc(n)+"</label>"}).join("");' +
      'return "<article class=product data-number=\\""+esc(p.productNum)+"\\"><h3>"+esc(p.name)+"</h3>' +
        '<small>#"+esc(p.productNum)+" - $"+Number(p.price).toFixed(2)+" - "+p.qty+" available</small>' +
        '<p>"+esc(p.description||"No description")+"</p>' +
        '<div class=photos>"+pics+"</div>' +
        '<div class=actions>' +
          '<input id=\\"add-"+esc(p.productNum)+"\\" type=file accept=\\"image/jpeg,image/png,image/webp\\" multiple>' +
          '<button class=secondary onclick=addPhotos(\'"+ esc(p.productNum)+"\')>Add photos</button>' +
          '<button class=secondary onclick=removePhotos(\'"+ esc(p.productNum)+"\')>Remove selected</button>' +
          '<button class=danger onclick=sold(\'"+ esc(p.productNum)+"\')>Mark sold</button>' +
          '<button onclick=edit(\'"+ esc(p.productNum)+"\')>Edit</button>' +
        '</div></article>"}).join("")||"<p>No products found.</p>"}' +
    'function edit(n){var p=products.filter(function(x){return x.productNum===n})[0];if(!p)return;$("title").textContent="Edit product #"+n;$("number").value=n;$("name").value=p.name;$("description").value=p.description||"";$("price").value=p.price;$("qty").value=p.qty;$("type").value=p.type||"cutting-board";window.scrollTo(0,0)}' +
    'function read(fs,done){var a=[],left=fs.length;if(!left)return done(a);Array.prototype.forEach.call(fs,function(f){var r=new FileReader();r.onload=function(){a.push({name:f.name,mimeType:f.type,base64:String(r.result).split(",")[1]});if(!--left)done(a)};r.readAsDataURL(f)})}function addPhotos(n){read($("add-"+n).files,function(a){if(!a.length){status("Select photos first");return}status("Adding photos...");call("addInventoryAdminImages",[n,a,KEY],function(){status("Photos added");load()})})}function removePhotos(n){var c=document.querySelector("[data-number=\\""+n+"\\"]"),a=Array.prototype.map.call(c.querySelectorAll("input:checked"),function(x){return x.value});if(!a.length||!confirm("Remove selected photos?"))return;call("removeInventoryAdminImages",[n,a,KEY],function(){status("Photos removed");load()})}function sold(n){if(!confirm("Mark product #"+n+" as sold?"))return;call("markInventoryAdminProductSold",[n,KEY],function(){status("Product marked sold");load()})}$("form").onsubmit=function(e){e.preventDefault();var d={productNum:$("number").value.trim(),name:$("name").value.trim(),description:$("description").value.trim(),price:$("price").value,qty:$("qty").value,type:$("type").value};read($("files").files,function(a){d.images=a;status("Saving...");call("saveInventoryAdminProduct",[d,KEY],function(){status("Product saved");load()})})};$("clear").onclick=function(){$("form").reset();$("title").textContent="Add product"};load();</script></body></html>';
}
